...
def tobvalid(i, o=None, m=1, p=None):
    ...
    data_hist = hv.Histogram(np.histogram(data, bins='scott', density=True)).opts(
        xlabel="Atomic B Values", width=500, height=500, shared_axes=False)
    p_data_hist = hv.Histogram(np.histogram(p_data, bins='scott', density=True)).opts(
        xlabel="Peak Height Values", width=500, height=500, shared_axes=False)
    layout = pn.Column("# Program could not analyse the data. The reason is: {} ".format("Too many values close to minimum: Possible oversharpening case."),
                       "## Please look at the histograms of ADP values and Peak Height distributions",
                       "### {}".format(file_name),
    ...
    layout.save("{}.html".format(file_name), resources=INLINE)
