...
def atan(Bval, Bn, smax, ml):
    ...
    if bmedian > 0:
        ...
        ccm = (b02/b01)**1.5
        if smax > 0:
            cc1 = np.sqrt(np.pi)*erf(np.sqrt(b01)*smax/2) - \
                np.sqrt(b01)*smax*np.exp(-b01*smax**2/4)
            cc2 = np.sqrt(np.pi)*erf(np.sqrt(b02)*smax/2) - \
                np.sqrt(b02)*smax*np.exp(-b02*smax**2/4)
            ccm = ccm*cc1/cc2
        iqr = bq3-bq1
...
def local_analysis(input_path, ouput_path):
    ...
    for i in range(nB):
        ...
        if ml >= 3:
            ...
            if ccq1 > 1.01 and ccm > 1.2:
                F = open(unus_path, "a")
                F.write(">> Potentially heavier atom with occupancy = "+str(round(float(ccm), 2))+"\n summ   "+str(ml)+"  "+str(round(float(bav), 2))+"  "+str(round(float(bst), 2))+"  "+str(round(float(bz), 2))+"  "+str(round(float(bmedian), 2))+"   "+str(round(
                    float(bq1), 2))+"  "+str(round(float(bq3), 2))+"  "+str(round(float(ccm), 2))+"  "+str(round(float(ccq1), 2))+"  "+str(round(float(ccq3), 2))+"\natom "+str(i+1)+" "+ta+" . "+tr+" "+tri+" . "+tc+"    "+str(round(Bval, 2))+"    "+str(round(s, 2))+"\n")
                F.close()
            ...
            if tr == 'HOH':
                if ml >= 8:
                    ...
                    F2.write("atom "+str(i+1)+"  "+ta+"  "+tr+"  "+tri +
                             "  "+tc+"  "+str(round(Bval, 2))+"  .:. "+str(ml)+"\n")
                    F2.close()
